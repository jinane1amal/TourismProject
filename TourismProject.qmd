...
In this section, we want to study the impact of all the variables on our main variables by using a linear regression. To do so, we will first focus on the impact in 2019 and after that in 2020.   
...
Before doing the linear regression we merge our data set using the following code :
...
$$Arrivals=\beta_0+\beta_1AverageExp+\beta_2Food+\beta_3Forest+\beta_4Happy+\beta_5GDP$$
...
We notice that only a single explanatory variable is significant : `2019_arrivals`.   
...
Once again, the only significant variable is `2020_arrivals`.  
    

However, one should pay attention to the value of our adjusted $R^2$. In fact, in both cases, $R^2$ is not high enough (respectively $0.382$ and $0.283$). Thus, maybe this linear regression with thoses explanatory vriable is not the best, maybe the unsignificant variable are downgrading the quality of our model, suggesting that those variables may not explain the number of arrivals in a country.  
