...
We observe that, for both regression, the p-value is higher than $0.05$, we won't reject the null hypothesis. In fact, this result could have been more thant predicatble since our regression is not temporal and it depends on different countries, so their results and data are not correlated ith each other.  

# IX. Final Scatter plots: 


```{r}
scatterplot(Tab$'2019_arrivals'~ Tab$'2019_avg', data=Tab) 
```


```{r}
scatterplot(Tab$'2020_arrivals'~ Tab$'2020_avg', data=Tab) 
```
